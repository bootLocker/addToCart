var fs = require('fs');
// fs.writeFile('/home/mylonjones/hackreactor/addToCart/db/tenMillion.csv', 'shoeName,gender,price,size,currentShoeBigPictures,currentShoeSmallPictures,otherColorWays,colorway,fit,fitAlert,rating\n', (err) => {
//   if (err) { throw err; }
//   console.log('file overwritten');
// });
var ws = fs.createWriteStream('/home/mylonjones/hackreactor/addToCart/db/tenMillion.csv', { flags: 'a' });
...
var createRandomShoes = (amount) => {
  for (var i = 1; i <= amount; i++) {
    var currentShoe = '';
    currentShoe += shoeName();
    currentShoe += gender();
    currentShoe += price();
    currentShoe += size();
    currentShoe += currentShoeBigPictures();
    currentShoe += currentShoeSmallPictures();
    currentShoe += otherColorWays();
    currentShoe += colorway();
    currentShoe += fit();
    currentShoe += fitAlert();
    currentShoe += rating();

    ws.write(currentShoe);
  }
};
